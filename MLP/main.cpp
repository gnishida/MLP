...
int main(int argc,char *argv[]) {
	...
	*/
	...
	cv::Mat_<double> normalized_X, muX, maxX, normalized_Y, muY, maxY;
	myutil::normalize(X, normalized_X, muX, maxX);
	myutil::normalize(Y, normalized_Y, muY, maxY);

	cv::Mat_<double> trainX, trainY, validX, validY, testX, testY;
	myutil::split(normalized_X, 0.8f, 0.1f, trainX, validX, testX);
	myutil::split(normalized_Y, 0.8f, 0.1f, trainY, validY, testY);
	...
	MLP best_mlp(trainX.cols, 1, trainY.cols);
		...
		for (int j = 0; j < alpha.size(); ++j) {
			for (int k = 0; k < hiddenSize.size(); ++k) {
				for (int l = 0; l < maxIter.size(); ++l) {
					...
					//for (int m = 0; m < 100; ++m) {
						MLP mlp(trainX.cols, hiddenSize[k], trainY.cols);
						mlp.init();
	...
	double score2 = best_mlp.cost(validX, validY, 0);
	cout << " again, score: " << score2 << endl;
	...
	for (int i = 0; i < testX.rows; ++i) {
		cv::Mat_<double> y = best_mlp.predict(testX.row(i));

		for (int j = 0; j < testY.cols; ++j) {
			error += abs(y(0, j) - testY(i, j));
		}

		cout << y(0, 0) << ", " << testY(i, 0) << ", " << abs(y(0, 0) - testY(i, 0)) << endl;
	}
	cout << "Avg error: " << error / testX.rows / testY.cols << endl;
